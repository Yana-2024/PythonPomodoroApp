import time #to wait time before countdown
import threading #use diff threads to be able to accress buttons while countdown
import tkinter as tk #library for GUI (button,label,entry,frame)
from tkinter import ttk, PhotoImage #themed tkinter (themes,colors,fonts)


class PomodoroTimer: #UI
    def __init__(self):#UI 
        self.root = tk.Tk()
        self.root.geometry("500x250") #size of window
        self.root.title("Pomodoro Timer") #title of app
        self.root.tk.call('wm','iconphoto',self.root._w,PhotoImage(file="pomodoro tomato.png")) #presented labels in window
        ...
        self.pomodoro_timer_label=ttk.Label(self.tab1,text="25:00",font=("Ubuntu",30))
        self.pomodoro_timer_label.pack(pady=20)

        self.short_break_timer_label=ttk.Label(self.tab2,text="05:00",font=("Ubuntu",30))
        self.short_break_timer_label.pack(pady=20)

        self.long_break_timer_label=ttk.Label(self.tab3,text="15:00",font=("Ubuntu",30))
        ...
        self.root.mainloop() #loop to keep the window showing
    ...
    def start_timer(self):
        ...
        if timer_id==1:
            full_seconds=60*25 #convert the minutes to seconds
            ...
            full_seconds=60*5 #mins to secs
            while full_seconds>0 and not self.stopped:
                minutes,seconds=divmod(full_seconds,60)
            ...
            full_seconds=60*15 #mins tro secs
            while full_seconds>0 and not self.stopped:
                minutes,seconds=divmod(full_seconds,60)
